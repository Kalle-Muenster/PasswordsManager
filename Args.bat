...
if "%~1"=="GUI" set _target_=%~6
if "%~2"=="GUI" set _target_=%~6
if "%~3"=="GUI" set _target_=%~6

:CHECK_FOURTH
if "%~4"=="x86" set _arch_=x86
if "%~4"=="Debug" set _conf_=Debug
if "%~4"=="Clean" set _clean_= /t:Clean
if "%~4"=="GUI" set _target_=%~6
...
if "%_target_%"=="%~6" set _gui_=\GUI

:SET_BUILD_CALL
set _build_=%_root_%\bin\%DotNetVersionString%%_gui_%
if not "%_call_%"=="%_here_%" (
  if not "%_call_%"=="%_root_%" (
    set _build_=%_call_%%_gui_%
  )
)
